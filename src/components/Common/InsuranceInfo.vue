    ...
    <div class="column-5 dashed-border">
      <InputGroup class=" w-full mb-2.5" title="姓名">
        <div slot="input" class="w-full pr-24 relative">
          ...
          <Button class="absolute right-0 -top-1 w-10 md:w-16 h-full" style="height: 50px" @click.native="() => { if(copyInfo.Name) {$emit('getDetail', 'Name')}}">查詢</Button>
        </div>
      </InputGroup>
      <InputGroup class=" w-full mb-2.5" title="統編/身分證">
        <div slot="input" class="w-full pr-24 relative">
          ...
          <Button class="absolute right-0 -top-1 w-10 md:w-16 h-full" style="height: 50px" @click.native="() => { if(copyInfo.ID) {$emit('getDetail', 'ID')}}">查詢</Button>
        </div>
      </InputGroup>
    </div>
