  ...
  <div>
    ...
    <div class="column-6 pb-3 my-4">
      <InputGroup class="w-full col-span-6" title="投保行業" v-if="copyInfo.CorporateRequired || true" borderBtn :editModel="editModel">
        ...
        <div v-else slot="input" class="w-full relative">
          {{info.displayInsureType && info.displayInsureType.includes('其他')? `${info.displayInsureType}(${info.otherIndustryName})`: (info.otherIndustryName ? info.otherIndustryName : info.displayInsureType)}}
        </div>
      </InputGroup>
    </div>
