      ...
      <div class="body" ref="body" @scroll="scroll">
        ...
        <div class="fixed-button" v-if="!audit">
          <div class="flex justify-center w-full px-3">
            ...
            <Button data-cy="填寫完成" v-if="questionnaireType == 0" class="h-12 w-52 mr-3" @click.native="() =>{$store.dispatch('activity/updateQuestionnaireFinished', true);$emit('update:open' ,false)}">{{(windowWidth > 600) ? '填寫完成' : '完成'}}</Button>
